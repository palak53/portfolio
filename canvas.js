...
function floatySpace() {
  ...
  space = new CanvasSpace("canvas", "	#181818" ).display();
  var form = new Form( space );

  // Elements
  var pts = [];
  var center = space.size.$divide(2.3);
  var angle = -(window.innerWidth * 0.5);
  var count = window.innerWidth * 5.5;
  if (count > 150) count = 150;
  var line = new Line(0.5, angle).to(space.size.x, 10);
  var mouse = center.clone();

  var r = Math.min(space.size.x, space.size.y) * 1.8;
